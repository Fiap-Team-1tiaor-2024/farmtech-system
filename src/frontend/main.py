...
st.title("🌱 Farmtech")
st.caption("Farmtech é uma plataforma de gestão agrícola que permite o gerenciamento de culturas, propriedades e produções.")

st.markdown("---")
st.caption("Clique no botão abaixo para criar uma cultura.")

st.write("**Criar uma cultura**")
cultura_nome = st.text_input("Nome da Cultura", placeholder="Digite uma cultura (Ex: Milho)", key="cultura_nome_input")
if st.button("Criar", key="criar_cultura_btn"):
    response = requests.post("http://backend:8080/v1/farmtech/cultura", json={"nome": cultura_nome})
...
propriedade_nome = st.text_input("Nome da Propriedade", placeholder="Digite o nome da propriedade", key="propriedade_nome_input")
propriedade_localizacao = st.text_input("Localização da Propriedade", placeholder="Informe a localização da propriedade", key="propriedade_localizacao_input")
if st.button("Criar Propriedade", key="criar_propriedade_btn"):
    response = requests.post("http://backend:8080/v1/farmtech/propriedade", json={"nome": propriedade_nome, "localizacao": propriedade_localizacao})
    if response.status_code == 201:
...
producao_cultura_id = st.number_input("ID da Cultura para Produção", placeholder="Digite o ID da cultura", min_value=1, key="producao_cultura_id_input")
producao_propriedade_id = st.number_input("ID da Propriedade para Produção", placeholder="Informe o ID da propriedade", min_value=1, key="producao_propriedade_id_input")
producao_area = st.number_input("Área da Produção", placeholder="Digite a área da produção", min_value=0.0, format="%.2f", key="producao_area_input")
producao_custo = st.number_input("Custo da Produção", placeholder="Digite o custo da produção", min_value=0.0, format="%.2f", key="producao_custo_input")
...
if st.button("Criar Produção", key="criar_producao_btn"):
    ...
    if not all(payload.values()): 
        ...
        response = requests.post("http://backend:8080/v1/farmtech/producao", json=payload)
        if response.status_code == 201:

...
CSV_OUTPUT_DIR_RELATIVE_TO_APP = "estatistica/r/csv" # Usado para servir os arquivos via GET
...
def get_csv(filename: str):
    ...
    if not os.path.exists(file_path):
        ...
        print(f"Conteúdo de {ABSOLUTE_CSV_OUTPUT_DIR}: {os.listdir(ABSOLUTE_CSV_OUTPUT_DIR) if os.path.exists(ABSOLUTE_CSV_OUTPUT_DIR) else 'Diretório não existe'}")
        raise HTTPException(status_code=404, detail=f"Arquivo '{filename}' não encontrado em '{ABSOLUTE_CSV_OUTPUT_DIR}'. Verifique se o script R foi executado e gerou os arquivos.")

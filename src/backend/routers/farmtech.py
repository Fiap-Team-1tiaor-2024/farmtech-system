...
@router.post(
    '/cultura',
    response_model=schemas.Cultura,
    status_code=status.HTTP_201_CREATED,
)
def criar_cultura(
    cultura: schemas.CulturaCreate, db: Session = Depends(get_db)
):
...
@router.get(
    '/cultura',
    response_model=list[schemas.Cultura],
...
@router.post(
    '/propriedade',
    response_model=schemas.Propriedade,
    status_code=status.HTTP_201_CREATED,
)
def criar_propriedade(
    propriedade: schemas.PropriedadeCreate, db: Session = Depends(get_db)
):
...
@router.get(
    '/propriedade',
    response_model=list[schemas.Propriedade],
    status_code=status.HTTP_200_OK,
)
...
@router.post(
    '/producao',
    response_model=schemas.Producao,
    status_code=status.HTTP_201_CREATED,
)
def criar_producao(
    producao: schemas.ProducaoCreate, db: Session = Depends(get_db)
):
...
@router.get(
    '/producao',
    response_model=list[schemas.Producao],
    status_code=status.HTTP_200_OK,
)
